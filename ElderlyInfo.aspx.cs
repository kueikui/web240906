    ...
    public partial class ElderlyInfo : System.Web.UI.Page
    {
        ...
        protected void BindGridView()//gridview elder
        {
            ...
            else if(Session["LoginType"] == "Home")
            {
                MySqlConnection connection = new MySqlConnection(connectionString);
                connection.Open();
                string query = "SELECT HomeElder.heId as 長者編號,HomeElder.heName as 長者姓名, HomeElder.heGender as 長者性別,HomeElder.heBirth as 長者生日,HomeElder.heIdCard as 長者身分證,HomeElder.hePhone as 長者電話,HomeElder.heAddress as 長者地址,HomeElder.heHeight as 長者身高,HomeElder.heWeight as 長者體重 FROM HomeElder";
        ...
        protected void BindGridView2(string eId)//gridview fall
        {
            ...
            else if (Session["LoginType"] == "Home")
            {
                ...
                string query = "SELECT HomeElderFall.hfId as 跌倒編號,HomeElderFall.heId as 長者編號,HomeElder.heName as 長者姓名 ,HomeElderFall.pId as 跌倒地點,HomeElderFall.hfTime as 跌倒時間, HomeElderFall.hfWhy as 跌倒原因 ,HomeElderFall.hId as 送醫 FROM HomeElderFall " +
                           "INNER JOIN HomeElder ON HomeElderFall.heId = HomeElder.heId WHERE HomeElderFall.heId = @heId";
                MySqlCommand command = new MySqlCommand(query, connection);
                command.Parameters.AddWithValue("@heId", eId);
                MySqlDataAdapter adapter = new MySqlDataAdapter(command);
                DataTable dt = new DataTable();
                adapter.Fill(dt);
                GridView2.DataSource = dt;
        ...
        protected void edit_Click(object sender, EventArgs e)//編輯
        {
            ...
            {
                if (cId_Text.Text == Session["userName"].ToString())
                {
                    edit.Visible = false;
                    save.Visible = true;
                    back.Visible = false;
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)//長者詳細資料
        {
            ...
            else if (Session["LoginType"] == "Home")
            {
                ...
                if (e.CommandName == "Details")
                {
                    ...
                    eName_Text.Text = selectedRow.Cells[3].Text;//HomeElder.heName as 長者姓名
                    //eGender_list.Text = eGenderCell.Text; HomeElder.heGender as 長者性別
                    string dateString1 = selectedRow.Cells[4].Text;
                    
                    eIdCard_Text.Text = selectedRow.Cells[6].Text;//HomeElder.heIdCard as 長者身分證
                    ePhone_Text.Text = selectedRow.Cells[7].Text;//HomeElder.hePhone as 長者電話
                    eAddress_Text.Text = selectedRow.Cells[8].Text;//HomeElder.heAddress as 長者地址
                    eHeight_Text.Text = selectedRow.Cells[9].Text;//HomeElder.heHeight as 長者身高
                    eWeight_Text.Text = selectedRow.Cells[10].Text;//HomeElder.heWeight as 長者體重
                    //cId_Label.Text = "照顧者:";
        ...
        protected void Add_Click(object sender, EventArgs e)//新增畫面
        {
            ...
            else if (Session["LoginType"] == "Home")
            {
                ...
                MySqlConnection connection = new MySqlConnection(connectionString);
                connection.Open();
                string query = "select homeName from HomeLogin";
                MySqlCommand command = new MySqlCommand(query, connection);
                MySqlDataAdapter adapter = new MySqlDataAdapter(command);
                DataSet ds = new DataSet();
                adapter.Fill(ds, "cIdlist");
                cId_list.DataValueField = "homeName";        //在此輸入的是資料表的欄位名稱
                cId_list.DataTextField = "homeName";      //在此輸入的是資料表的欄位名稱
                ...
                else if (Session["LoginType"] == "Home")
                {
                    ...
                    if (reader.HasRows)
                    {
                        while (reader.Read())
                        {
                            ...
                            if (eId_Text.Text == reader.GetString("heId"))
                            {
                                Label1.Visible = true;
                                Label1.Text = "新增失敗，已有此帳號!";
                                return;
                            }
                            else if (ePhone_Text.Text == reader.GetString("hePhone"))
                            {
                                Label1.Visible = true;
                                Label1.Text = "新增失敗，已電話已綁定!";
                                return;
                            ...
                            {
                                query = "INSERT INTO HomeElder (heId,heName,heIdCard,heGender,heBirth,hePhone,heAddress,heHeight,heWeight,homeName) VALUES (@heId,@heName,@heIdCard,@heGender,@heBirth,@hePhone,@heAddress,@heHeight,@heWeight,@homeName)";
                                connection = new MySqlConnection(connectionString);
                                command = new MySqlCommand(query, connection);
                                connection.Open();
                                ...
                                command.Parameters.AddWithValue("@heHeight", eHeight_Text.Text);
                                command.Parameters.AddWithValue("@heWeight", eWeight_Text.Text);
                                command.Parameters.AddWithValue("@homeName", cId_list.Text);
                                command.ExecuteNonQuery();
                                connection.Close();
        ...
        protected void Fsave_Click(object sender, EventArgs e)
        {
            ...
            MySqlConnection connection = new MySqlConnection(connectionString);
            connection.Open();
            string query = "UPDATE Fall SET feName = @feName,fTime=@fTime,pId=@pId,fWhy=@fWhy,hId=@hId  WHERE fId = @fId";
            MySqlCommand command = new MySqlCommand(query, connection);
            command.Parameters.AddWithValue("@feName", feName_Text.Text);
            command.Parameters.AddWithValue("@fTime", Convert.ToDateTime(fTime_Text.Text));///////////////////////////////errorrrrrr
